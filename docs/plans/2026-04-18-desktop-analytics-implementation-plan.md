...
1. 默认开启有助于更真实地理解桌面版的使用情况、稳定性与性能表现。
2. 数据分析仍应保持脱敏与最小化，避免收集对象级敏感信息。
3. 更新检查属于产品基本可维护能力，默认开启更符合分发型桌面应用诉求。
4. 两类行为语义明确拆分后，更容易向用户解释。
...
- [ ] 在桌面外链打开、原生文件动作、路径动作、剪贴板动作中增加低敏感事件。
- [ ] 增加少量稳定性事件，例如 `ui_error`。
- [ ] 在不暴露对象级信息的前提下开启性能指标采集。

### 验收标准

- [ ] 首批事件控制在低敏感、高价值的有限范围内，避免无边界扩张。
